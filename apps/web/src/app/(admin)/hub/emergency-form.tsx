...
export function EmergencyForm({ barangayId, item }: { barangayId: string; item?: EmergencyInformation }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="grid grid-cols-2 gap-4">
      ...
      <label className="col-span-2 text-sm">
        <span className="mb-1 block font-medium">Content (one item per line)</span>
        {category === 'guidelines' ? (
          <p className="mb-1 text-xs text-zinc-500">Each line becomes a bullet point.</p>
        ) : (
          <p className="mb-1 text-xs text-zinc-500">Format: Label | Number (e.g. Police | 911)</p>
        )}
        <textarea
          className={inputClass}
          value={contentLines}
      ...
      <label className="col-span-2 text-sm">
        ...
        <input
          ...
          placeholder="information-circle-outline"
        />
      </label>
