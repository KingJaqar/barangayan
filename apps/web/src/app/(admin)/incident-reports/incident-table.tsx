...
export const INCIDENT_STATUS: Record<string, { label: string; className: string }> = {
  ...
  resolved: { label: 'Resolved', className: 'bg-[#0F6E5B]/15 text-[#0F6E5B]' },
...
export function IncidentTable({
  ...
  const columns: EditableDataTableColumn<IncidentRow>[] = [
    {
      ...
      render: (r) => (
        <div>
          ...
          {r.description && (
            <p className="mt-0.5 line-clamp-2 text-xs text-zinc-500 dark:text-zinc-400">{r.description}</p>
          )}
      ...
      render: (r) => (
        <div className="flex flex-wrap items-center gap-1" onClick={(e) => e.stopPropagation()}>
          <button
            ...
            className="rounded-full px-2 py-1 text-xs font-medium text-[#0F6E5B] hover:underline">
            View
          </button>
  ...
  return (
    <>
      <AddIncidentForm categories={categories} residents={residents} barangayId={barangayId} />
      <EditableDataTable
        rows={rows}
        rowKey={(r) => r.id}
