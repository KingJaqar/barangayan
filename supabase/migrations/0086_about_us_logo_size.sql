-- ============================================================================
-- Add logo_size to about_us + update RPC — migration 0086
-- ============================================================================
-- Stores the display size of the barangay logo (in pixels, 40–200) so admins
-- can control how large the logo appears on the About Us page.

alter table public.about_us
  add column logo_size integer not null default 64
    check (logo_size between 40 and 200);

-- Update the atomic save RPC to accept and persist logo_size.
...
begin
  ...
  insert into public.about_us
    (barangay_id, mission, vision, history, contact_email, contact_phone, address,
     logo_url, logo_size, is_active, sort_order)
  values
    (p_barangay_id, p_mission, p_vision, p_history, p_contact_email, p_contact_phone, p_address,
...
grant execute on function public.save_about_us_with_developers(
  uuid, text, text, text, text, text, text, text, jsonb, boolean, integer, integer
) to authenticated;
